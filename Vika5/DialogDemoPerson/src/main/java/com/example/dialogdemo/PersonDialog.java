...
public class PersonDialog extends Dialog<Person> {
	...
	public PersonDialog(Person person) {
		super(); // kallar fyrst á smið í Dialog klasanum
		this.setTitle("Add Person");
		this.person = person;
		buildUI(); // býr til dialog-inn
		setPropertyBindings(); // setur upp bindings við dialoginn
		setResultConverter();  // setur upp result converter
	}
	...
	private void setResultConverter() {
		Callback<ButtonType, Person> personResultConverter = param -> {
            if (param == ButtonType.OK) {
                return person; // skilar vinnsluhlut
            } else {
                return null;
            }
        };
